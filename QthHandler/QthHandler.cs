...
public partial class QthHandler : IQthHandler
{
    ...
    private ScanInfo _lastKeywordScan = new() { Date = DateTime.MinValue, Ids = new List<int>(), OtherIds = new List<int>() };
    private ScanInfo _lastCategoryScan = new() { Date = DateTime.MinValue, Ids = new List<int>(), OtherIds = new List<int>() };
    private ScanInfo _thisScan;
    private List<Post> _newPosts;

    // ReSharper disable once SuggestBaseTypeForParameter
    public QthHandler(ILogger<HostedService> logger, IOptions<AppSettings> settings)
    {
        _logger = logger;
    ...
    private static Post ProcessPost(string html, ScanType scanType)
    {
        ...
        var title = "<font size=2 face=arial> - ";
